...
# Loads ACCESS_TOKEN and USER_NAME from a local .env file (never commit this file)
load_dotenv()
...
def graph_repos_stars(
    ...
    """
    Uses GitHub's GraphQL v4 API to return my total repository,
    star, or lines of code count.
    """
...
def loc_counter_one_repo(
    ...
    """
    Recursively call recursive_loc since GraphQL can only search
    100 commits at a time.
...
def loc_query(
    ...
    """
    Uses GitHub's GraphQL v4 API to query all the repositories
    I have access to.
...
def cache_builder(
    ...
    """
    Checks each repository in edges to see if it has been updated
    since the last time it was cached.

    If it has, run recursive_loc on that repository to update
    the LOC count.
    """
    ...
    # FORCE CACHE REFRESH
    #
    # If force_cache=True, every repository gets its LOC
    # recalculated from GitHub.
    ...
    for index in range(len(edges)):
        ...
        if repo_hash == hashlib.sha256(
            ...
            try:
                ...
                # Recalculate LOC if:
                # 1. The repository commit count changed
                # OR
                # 2. force_cache=True
...
def flush_cache(edges, filename, comment_size):
    ...
    or when the file is first created.
    """
...
def svg_overwrite(
    ...
    """
    Parse SVG files and update elements with my age, commits,
    stars, repositories, and lines written.
    """
...
def justify_format(
    ...
    """
    Updates and formats the text of the element and modifies
    the amount of dots in the previous element to justify
    the new text on the SVG.
    """
...
def commit_counter(comment_size):
    """
    Counts up my total commits using the cache file created
    by cache_builder.
    """
...
def formatter(
    ...
    Returns formatted result if whitespace is specified,
    otherwise returns raw result.
    """
...
if __name__ == '__main__':
    ...
    """
    Personal GitHub profile stats script
    Adapted from Andrew6rant/github-stats-transparent
    """
